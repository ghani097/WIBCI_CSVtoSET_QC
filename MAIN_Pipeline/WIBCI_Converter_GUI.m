...
% Date: January 2026
    ...
    % Trigger options
    appData.includeTriggers = false;
    appData.triggerEventA = false;
    appData.triggerEventB = false;
...
function createConverterUI(parent, fig)
    ...
    uicontrol('Parent', optionsPanel, ...
              'Style', 'checkbox', ...
              'Tag', 'includeTriggerCheck', ...
              'String', 'Include Triggers', ...
              'Value', 0, ...
              'Position', [20, 18, 130, 25], ...
              'FontSize', 9, ...
              'Callback', @includeTriggerChanged);

    uicontrol('Parent', optionsPanel, ...
              'Style', 'checkbox', ...
              'Tag', 'triggerEventACheck', ...
              'String', 'Event A', ...
              'Value', 0, ...
              'Position', [170, 18, 80, 25], ...
              'FontSize', 9, ...
              'Enable', 'off', ...
              'Callback', @triggerEventAChanged);

    uicontrol('Parent', optionsPanel, ...
              'Style', 'checkbox', ...
              'Tag', 'triggerEventBCheck', ...
              'String', 'Event B', ...
              'Value', 0, ...
              'Position', [260, 18, 80, 25], ...
              'FontSize', 9, ...
              'Enable', 'off', ...
              'Callback', @triggerEventBChanged);
...
function includeTriggerChanged(hObject, ~)
    fig = ancestor(hObject, 'figure');
    appData = guidata(fig);
    appData.includeTriggers = logical(get(hObject, 'Value'));

    eventACheck = findobj(fig, 'Tag', 'triggerEventACheck');
    eventBCheck = findobj(fig, 'Tag', 'triggerEventBCheck');

    if appData.includeTriggers
        set(eventACheck, 'Enable', 'on');
        set(eventBCheck, 'Enable', 'on');
        logMessage(fig, 'Triggers enabled. Select Event A and/or Event B.');
    else
        set(eventACheck, 'Enable', 'off', 'Value', 0);
        set(eventBCheck, 'Enable', 'off', 'Value', 0);
        appData.triggerEventA = false;
        appData.triggerEventB = false;
        logMessage(fig, 'Triggers disabled.');
    end

    guidata(fig, appData);
end

function triggerEventAChanged(hObject, ~)
    fig = ancestor(hObject, 'figure');
    appData = guidata(fig);
    appData.triggerEventA = logical(get(hObject, 'Value'));
    guidata(fig, appData);
end

function triggerEventBChanged(hObject, ~)
    fig = ancestor(hObject, 'figure');
    appData = guidata(fig);
    appData.triggerEventB = logical(get(hObject, 'Value'));
    guidata(fig, appData);
end
...
function startConversion(hObject, ~)
    ...
    if appData.includeTriggers
        trigSources = {};
        if appData.triggerEventA, trigSources{end+1} = 'Event A'; end
        if appData.triggerEventB, trigSources{end+1} = 'Event B'; end
        if isempty(trigSources)
            logMessage(fig, 'Triggers: Enabled but no event column selected');
        else
            logMessage(fig, sprintf('Triggers: %s', strjoin(trigSources, ', ')));
        end
    else
        logMessage(fig, 'Triggers: Disabled');
    end
...
function EEG = convertWiBCIFile(filePath, appData)
    ...
    EEG.event = eegLabEventStructW(softEventAVector, softEventBVector, ...
                                    ...
                                    softEventACodes, softEventBCodes, ...
                                    hardEventACodes, hardEventBCodes);
